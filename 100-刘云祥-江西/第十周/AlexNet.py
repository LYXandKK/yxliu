...
def AlexNet(input_shape=(224, 224, 3), output_shape=2):
    # AlexNet
    model = Sequential()  # 建立了一个 Sequential 模型，并取名为 model。Sequential模型只有一组输入和一组输出，各层之间按照先后顺序进行堆叠。前面一层的输出就是后面一次的输入
    ...
    # 缩减为1024
    model.add(Flatten())  # 将输入“压平”，即把多维的输入一维化，常用在从卷积层到全连接层的过渡
